---
name: phase-contract
description: 【PhaseFlow】Superpowers /review 通过后由 Stop hook 自动触发，生成该技术层的接口契约文件（contract），供下一层 brainstorming 注入使用。contract 包含接口签名、业务语义和约束来源，是层间与模块间上下文传递的核心载体。
---
...
由 Superpowers `/review` 的 **Stop hook 自动触发**，不需要手动执行。
