...
| PhaseFlow   | 流程层 | Phase 管理、状态追踪、契约传递 |
...
Superpowers /review
    ...
    → docs/contracts/{模块}-{data|service|api}.md
...
| `docs/contracts/{模块}-data.md`                   | 数据层接口契约                            | /phase-contract（手动，/review 后执行） |
| `docs/contracts/{模块}-service.md`                | 服务层接口契约（含对外接口/内部约定分区） | /phase-contract（手动，/review 后执行） |
| `docs/contracts/{模块}-api.md`                    | API 层接口契约                            | /phase-contract（手动，/review 后执行） |
...
- **Contract 在每层 /review 通过后手动执行 /phase-contract 生成**，不得跳过
